...
- **Canvas** — `{ id, name, nodes[], edges[] }`. The source of truth for the
  ...
  - **Edge** — `{ id, source, target, sourceHandle, targetHandle, type:'note',
    data:{ createdAt? } }`. `createdAt` (ms) records when the connection
    was made so trace mode stacks a note's connections — and `edgeNumbers`
    orders siblings — in the order they were drawn (older edges fall back to
    array order). Edges have
    no editable labels — `NoteEdge` shows an auto-computed outline number
    (1, 1.2, 2.1.3 …) derived from the graph (`edgeNumbers`, CanvasBoard);
    a legacy `data.label` may linger in old saves but is never shown.
...
| **ProjectView** | Main workspace shell. Owns the sidebar (canvases, library, search, export), routes between `CanvasBoard` and `DocumentReader`, remembers each canvas's viewport, and coordinates note⇄reader navigation (`sendToCanvas`, `viewHlOnCanvas`, `openSourceHl`, "back to note" chip). `convertDocToCanvas` turns a document into a new canvas via `lib/outline.js`. |
| **CanvasBoard** | The React Flow canvas. Owns nodes/edges state + debounced autosave, selection, keyboard shortcuts, add notes via a compact kind dropdown ("Add"), duplicate / link / group notes (⌘G groups, ⌘⇧G ungroups; a group selects, drags and deletes as one), box-select (Ctrl/Cmd-drag), image paste & drop (issue #24), auto-layout via a single "Arrange" dropdown (snowflake — radial rings around each component's best-connected note — first as the default, then dagre hierarchy ↓/→, crossing-minimizing grid, and circle; whole-canvas or selected-only; layouts only move nodes/re-anchor handles, never touch edge direction; during a focused trace the same dropdown restyles the trace view display-only, plus a "Trace columns" default for shallow traces), fit-all ("Fit" button broadcasts `FitAllContext` so every note auto-sizes to its content), trace mode (focus halo, neighbors wrapped into balanced columns in connection order, edges temporarily re-anchored to facing sides and elevated above cards — all display-only, restored on exit; the usual outline-number badges keep labeling edges), deep trace (⇧T or context menu: the full context closure — everything the note feeds plus every note feeding those, transitively — laid out left→right with dagre), a right-click node context menu (edit / duplicate / trace / deep trace / delete), filter view, recall mode, and viewport persistence. Wraps the board in `RecallContext`, `OpenSourceContext`, `NodeSizeContext`. |
...
| **NoteEdge** | Custom edge (bezier) with a selection halo and the auto outline number (`EdgeNumContext`) as a badge at its midpoint — the same numbering in and out of trace mode. Badges counter-scale against zoom (capped) and render above nodes and edges (label layer sits over even trace-elevated edges). |
...
| **DocumentReader** | Reads a library document: outline/TOC with scrollspy, text selection → highlight popover, highlights & annotations panel, reading-time clock, progress bar, send/view-highlight-on-canvas, a convert-to-canvas button (one note per section, connected topic → subtopic), and an edit mode (pencil in the header) that swaps the article for a markdown textarea — highlights re-anchor by quote + context after a save (issue #32). Restores the reading position on open. |
...
| **contexts.js** | `RecallContext`, `OpenSourceContext`, `NodeSizeContext`, `FitAllContext`, `EdgeNumContext`. |
...
| **lib/outline.js** | `outlineCanvas(doc, tag)` — split a document at ATX headings (fence-aware) into one note per section/sub-section, connected as the outline tree (topic → subtopic, recursively; a title note roots multi-`h1` docs), laid out in reading order with one indent column per level. Powers "convert to canvas". |
