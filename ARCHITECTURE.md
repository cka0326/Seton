...
- **Canvas** — `{ id, name, nodes[], edges[] }`. The source of truth for the
  ...
  - **Edge** — `{ id, source, target, sourceHandle, targetHandle, type:'note',
    ...
    order they were drawn (older edges fall back to array order). Edges have
...
| **CanvasBoard** | The React Flow canvas. Owns nodes/edges state + debounced autosave, selection, keyboard shortcuts, add notes via a compact kind dropdown ("Add"), duplicate / link / group notes (⌘G groups, ⌘⇧G ungroups; a group selects, drags and deletes as one), box-select (Ctrl/Cmd-drag), image paste & drop (issue #24), auto-layout via a single "Arrange" dropdown (dagre hierarchy ↓/→, crossing-minimizing grid, snowflake — radial rings around each component's best-connected note — and circle; whole-canvas or selected-only; layouts only move nodes/re-anchor handles, never touch edge direction; during a focused trace the same dropdown restyles the trace view display-only, plus a "Trace columns" default for shallow traces), fit-all ("Fit" button broadcasts `FitAllContext` so every note auto-sizes to its content), trace mode (focus halo, neighbors wrapped into balanced columns in connection order, edges temporarily re-anchored to facing sides, numbered, and elevated above cards — all display-only, restored on exit), deep trace (⇧T or context menu: the full context closure — everything the note feeds plus every note feeding those, transitively — laid out left→right with dagre), a right-click node context menu (edit / duplicate / trace / deep trace / delete), filter view, recall mode, and viewport persistence. Wraps the board in `RecallContext`, `OpenSourceContext`, `NodeSizeContext`. |
...
| **NoteEdge** | Custom edge (bezier) with a selection halo and a number badge at its midpoint: the auto outline number (`EdgeNumContext`), or the connection-order badge in trace mode (`data.traceOrder`). Badges counter-scale against zoom (capped) and render above nodes. |
